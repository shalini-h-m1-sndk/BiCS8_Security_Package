...
# SCRIPTER SCRIPT                : PQT_UT01_Common_Init.st3
# CVF CALL ALL SCRIPT            : None
# CVF SCRIPT                     : PQT_UT01_Common_Init.py
# DESCRIPTION                    : For default Intilization in Low Speed.
# PRERQUISTE                     : None
# STANDALONE EXECUTION           : Yes
# TEST ARGUMENTS                 : --test=PQT_UT01_Common_Init --isModel=false --enable_console_log=1 --adapter=0
...
class PQT_UT01_Common_Init(customize_log):
    ...
    def Run(self,globalParameters=None):
        ...
        """
        PQT_UT01_Common_Init using pattern generator
        """

        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "Card Intilization into LowSpeed/default speed\n")
        ...
        sdVolt.voltage = globalParameters["Volt"]
        ...
        sdVolt.regionSelect = sdcmdWrap.REGION_SELECT_TYPE.REGION_SELECT_PARTIAL_1V8
        self.__dvtLib.setVolt(sdVolt,statusReg,True)
        ...
        if (int(globalParameters["bus_width"]) == 0) or (int(globalParameters["bus_width"]) == 2):
...
class MainClass(TestCase.TestCase, customize_log):
    ...
    def test_PQT_UT01_Common_Init(self):
        obj = PQT_UT01_Common_Init(self.vtfContainer)
        obj.Run()
