...
# CVF SCRIPT                     : TC06_01_Maximum_CounterCheck.py
# DESCRIPTION                    : Verify Maximum counter of nonce.
# PRERQUISTE                     :
# STANDALONE EXECUTION           : Yes
# TEST ARGUMENTS                 : --test=TC06_01_Maximum_CounterCheck.py --isModel=false --enable_console_log=1 --adapter=0
# AUTHOR                         : Shalini HM
# REVIEWED BY                    : None
# DATE                           : 07-Nov-2024
################################################################################
...
class TC06_01_Maximum_CounterCheck(customize_log):
    ...
    def Run(self):
        ...
        counter_value=0
        # Counter is 4Byte
        MaxCounter=int("0xffffffff", 16)
        while(counter_value<MaxCounter):
            # Get Device challenge for LRMA Unlock
            Challenge = self.RMAutils.GetDeviceChallenge(rma='LRMA', signed=True)
            counter_value = Challenge.GetFourBytesToInt(0)

            if counter_value == MaxCounter:
                self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "Max counter value reached: %d" % counter_value)
                break

        Challenge = self.RMAutils.GetDeviceChallenge(rma='LRMA', signed=True)
        counter_value = Challenge.GetFourBytesToInt(0)        
        if counter_value>MaxCounter:
            raise ValidationError.TestFailError(self.fn, "Counter exceeded the max value: %d",counter_value)
        else:
            self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "Counter value already reached the defined limit.")
...
class MainClass(TestCase.TestCase, customize_log):
    ...
    def test_TC06_01_Maximum_CounterCheck(self):
        obj = TC06_01_Maximum_CounterCheck(self.vtfContainer)
        obj.Run()
