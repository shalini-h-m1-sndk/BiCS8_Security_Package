...
# CVF SCRIPT                     : TC06_02_Maximum_CounterCheck.py
# DESCRIPTION                    : Verify Maximum counter of nonce.
# PRERQUISTE                     :
# STANDALONE EXECUTION           : Yes
# TEST ARGUMENTS                 : --test=TC06_02_Maximum_CounterCheck.py --isModel=false --enable_console_log=1 --adapter=1
# AUTHOR                         : Shalini HM
# REVIEWED BY                    : None
# DATE                           : 07-Nov-2024
################################################################################
...
class TC06_02_Maximum_CounterCheck(customize_log):
    ...
    def Run(self):
        ...
        LRMA_Challenge=self.RMAutils.GetDeviceChallenge(rma='LRMA', signed=True)
        LRMA_counter = LRMA_Challenge.GetFourBytesToInt(0)

        if LRMA_counter == 1:
            self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "LRMA counter of nonce is reset to 1 after FW download")
        else:
            raise ValidationError.TestFailError(self.fn, "LRMA counter of nonce is not reset to 1 after Download. ")
...
class MainClass(TestCase.TestCase, customize_log):
    ...
    def test_TC06_02_Maximum_CounterCheck(self):
        obj = TC06_02_Maximum_CounterCheck(self.vtfContainer)
        obj.Run()
