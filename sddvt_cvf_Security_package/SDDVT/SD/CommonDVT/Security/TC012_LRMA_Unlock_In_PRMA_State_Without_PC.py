...
# CVF SCRIPT                     : TC012_LRMA_Unlock_In_PRMA_State_Without_PC.py
# DESCRIPTION                    : Performs LRMA Unlock in PRMA State without Power cycle.
# PRERQUISTE                     :
# STANDALONE EXECUTION           : Yes
# TEST ARGUMENTS                 : --test=TC012_LRMA_Unlock_In_PRMA_State_Without_PC.py --isModel=false --enable_console_log=1 --adapter=0
# AUTHOR                         : Shalini HM
# REVIEWED BY                    : None
# DATE                           : 07-Nov-2024
################################################################################
...
class TC012_LRMA_Unlock_In_PRMA_State_Without_PC(customize_log):
    ...
    def Run(self):
        ...
        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name,  "Get GDC for PRMA Unlock")
        self.__Challenge=self.RMAutils.GetDeviceChallenge(rma='PRMA', signed=True)

        # Unlock command for PRMA
        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name,  "Issue PRMA Unlock")
        self.RMAutils.UnlockCard("PRMA", self.__Challenge)

        # Get device challenge for PRMA
        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name,  "Get GDC for LRMA Unlock")
        try:
            self.Challenge = self.RMAutils.GetDeviceChallenge(rma='LRMA', signed=True)
            ...
            if error_code == "0x6000207":                
                ...
                self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "GDC CMD failed as expected,since card is in PRMA state")
        ...
        # Unlock command for LRMA
        try:
            status = self.RMAutils.UnlockCard("LRMA", self.__Challenge)
            if status != 0:
                ErrorCodes = str(hex(status))
                error_message = self.__SecurityErrorCodes.CheckError(ErrorCodes)
                if ErrorCodes == "0x6000207":                
                    self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, 
                                 f"Unlock unsuccessfull. Error Code: {ErrorCodes}, Message: {error_message}")
                    self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "Unlock unsuccessfull as expected,Trying to unlock an already unlocked card")
                    self.vtfContainer.CleanPendingExceptions()
                else:
...
class MainClass(TestCase.TestCase, customize_log):
    ...
    def test_TC012_LRMA_Unlock_In_PRMA_State_Without_PC(self):
        obj = TC012_LRMA_Unlock_In_PRMA_State_Without_PC(self.vtfContainer)
        obj.Run()
