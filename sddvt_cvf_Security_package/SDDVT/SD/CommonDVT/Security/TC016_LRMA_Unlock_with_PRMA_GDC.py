...
# CVF SCRIPT                     : TC016_LRMA_Unlock_with_PRMA_GDC.py
# DESCRIPTION                    : Performs PRMA Unlock with LRMA GDC
# PRERQUISTE                     :
# STANDALONE EXECUTION           : Yes
# TEST ARGUMENTS                 : --test=TC016_LRMA_Unlock_with_PRMA_GDC.py --isModel=false --enable_console_log=1 --adapter=0
# AUTHOR                         : Shalini HM
# REVIEWED BY                    : None
# DATE                           : 08-Nov-2024
################################################################################
...
class TC016_LRMA_Unlock_with_PRMA_GDC(customize_log):
    ...
    def Run(self):
        ...
        # PRMA Unlock with LRMA challenge
        try:
            ...
            if status != 0:
                    ...
                    if ErrorCodes == "0x6000205":                
                        ...
                        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "LRMA Unlock unsuccessfull as expected,with PRMA GDC")
        ...
        # issue GDC to check card is in lock status afer Unlock cmd failed
        try:
            self.Challenge=self.RMAutils.GetDeviceChallenge(rma='PRMA', signed=True)
            self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name,  "Card is in Locked state")
        except:
            raise ValidationError.TestFailError(self.fn,"GDC failed") 
...
class MainClass(TestCase.TestCase, customize_log):
    ...
    def test_TC016_LRMA_Unlock_with_PRMA_GDC(self):
        obj = TC016_LRMA_Unlock_with_PRMA_GDC(self.vtfContainer)
        obj.Run()
