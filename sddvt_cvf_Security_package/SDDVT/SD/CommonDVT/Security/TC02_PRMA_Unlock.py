...
# CVF SCRIPT                     : TCO2_PRMA_Unlock.py
# DESCRIPTION                    : Performs  PRMA Unlock and verify lock after Power Cycle.
# PRERQUISTE                     :
# STANDALONE EXECUTION           : Yes
# TEST ARGUMENTS                 : --test=TC02_PRMA_Unlock.py --isModel=false --enable_console_log=1 --adapter=0
# AUTHOR                         : Shalini HM
# REVIEWED BY                    : None
# DATE                           : 06-Nov-2024
################################################################################
...
class TC02_PRMA_Unlock(customize_log):
    ...
    def Run(self):
        ...
        # Unlock command
        #self.RMAutils.Unlock(self.__Challenge, rmaOption='PRMA', corrupt=False, signed=True)
        ...
        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name,  "Issue Diag to check Device State after PRMA Unlock")
        state = self.RMAutils.GetDeviceState(signed=True)
        
        if state == "Locked":
            self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name,  "After PRMA unlock, Card state will be Locked only. ")
        else:
            self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name,"PRMA Unlock Unsuccessful")
...
class MainClass(TestCase.TestCase, customize_log):
    ...
    def test_TC02_PRMA_Unlock(self):
        obj = TC02_PRMA_Unlock(self.vtfContainer)
        obj.Run()
