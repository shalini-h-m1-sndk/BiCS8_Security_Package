...
# CVF SCRIPT                     : TC013_LRMA_Unlock_In_LRMA_State.py
# DESCRIPTION                    : Performs LRMA Unlock in LRMA state
# PRERQUISTE                     :
# STANDALONE EXECUTION           : Yes
# TEST ARGUMENTS                 : --test=TC013_LRMA_Unlock_In_LRMA_State.py --isModel=false --enable_console_log=1 --adapter=0
# AUTHOR                         : Shalini HM
# REVIEWED BY                    : None
# DATE                           : 07-Nov-2024
################################################################################
...
class TC013_LRMA_Unlock_In_LRMA_State(customize_log):
    ...
    def Run(self):
        ...
        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name,  "Get Device challenge for LRMA Unlock")
        self.__Challenge=self.RMAutils.GetDeviceChallenge(rma='LRMA', signed=True)

        # Unlock command for LRMA
        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name,  "Unlock command for LRMA")
        self.RMAutils.UnlockCard("LRMA", self.__Challenge)

        # Get device challenge for LRMA
        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name,  "Get Device challenge for LRMA Unlock")
        try:
            self.__Challenge = self.RMAutils.GetDeviceChallenge(rma='LRMA', signed=True)
        ...
        try:
            ...
            if status != 0:
                ...
                if ErrorCodes == "0x600020b":                
                    self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, 
                                 f"Unlock unsuccessfull. Error Code: {ErrorCodes}, Message: {error_message}")
                    self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "Unlock unsuccessfull as expected,Diag is not allowed in LRMA unlcok mode")
                    self.vtfContainer.CleanPendingExceptions()
                else:
...
class MainClass(TestCase.TestCase, customize_log):
    ...
    def test_TC013_LRMA_Unlock_In_LRMA_State(self):
        obj = TC013_LRMA_Unlock_In_LRMA_State(self.vtfContainer)
        obj.Run()
