...
# CVF SCRIPT                     : TC014_PRMA_Unlock_In_PRMA_State.py
# DESCRIPTION                    : Performs PRMA Unlock in PRMA state.
# PRERQUISTE                     :
# STANDALONE EXECUTION           : Yes
# TEST ARGUMENTS                 : --test=TC014_PRMA_Unlock_In_PRMA_State.py --isModel=false --enable_console_log=1 --adapter=0
# AUTHOR                         : Shalini HM
# REVIEWED BY                    : None
# DATE                           : 07-Nov-2024
################################################################################
...
class TC014_PRMA_Unlock_In_PRMA_State(customize_log):
    ...
    def Run(self):
        ...
        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name,  "Get Device challenge for PRMA Unlock")
        self.__Challenge=self.RMAutils.GetDeviceChallenge(rma='PRMA', signed=True)

        # Unlock command for PRMA
        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name,  "Unlock command for PRMA")
        ...
        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name,  "Unlock command for PRMA")
        try:
            status = self.RMAutils.UnlockCard("PRMA", self.__Challenge)
            if status != 0:
...
class MainClass(TestCase.TestCase, customize_log):
    ...
    def test_TC014_PRMA_Unlock_In_PRMA_State(self):
        obj = TC014_PRMA_Unlock_In_PRMA_State(self.vtfContainer)
        obj.Run()
