...
# CVF SCRIPT                     : CMD23_UT05_CMD20Write.py
# DESCRIPTION                    :
# PRERQUISTE                     : [CMD23_UT07_LoadCMD20_Variables,CMD23_UT03_CMD20_GetSequence]
...
class CMD23_UT05_CMD20Write(customize_log):
    ...
    def Run(self, WriteType = 0, StartBlock = 0x0, BlockCount = 0x1, DataType = 1, usePreDefBlkCount = False,
        ...
        if WriteType == 1:
            if Random == 0: # Multi Write
                self.__dvtLib.WriteWithFPGAPattern(StartLba =cmd20variables['DIRconstAddress'], blockCount = 0x1, SingleWrite =  False, pattern=2, usePreDefBlkCount= usePreDefBlkCount)
                self.__dvtLib.ReadWithFPGAPattern(StartLba =cmd20variables['DIRconstAddress'], blockCount = 0x1, SingleRead =  False, pattern=2, usePreDefBlkCount = usePreDefBlkCount)
                self.__GetSequence.Run(ExpectSequence)

            if Random == 1:  # Single Write
                self.__dvtLib.WriteWithFPGAPattern(StartLba = cmd20variables['FatRandomAddress'], blockCount = 0x1, SingleWrite =  True, pattern=2, usePreDefBlkCount = usePreDefBlkCount)
                self.__dvtLib.ReadWithFPGAPattern(StartLba = cmd20variables['FatRandomAddress'], blockCount = 0x1, SingleRead =  True, pattern=2, usePreDefBlkCount = usePreDefBlkCount)
                self.__GetSequence.Run(ExpectSequence)
        ...
        if WriteType == 2:
            ...
            self.__dvtLib.WriteWithFPGAPattern(StartLba =StartLBA, blockCount = BlockCount, SingleWrite =  False, pattern=2, usePreDefBlkCount = usePreDefBlkCount)
            self.__dvtLib.ReadWithFPGAPattern(StartLba =StartLBA, blockCount = BlockCount, SingleRead =  False, pattern=2, usePreDefBlkCount = usePreDefBlkCount)
            self.__GetSequence.Run(ExpectSequence)
        ...
        if WriteType == 3:
            ...
            if Random == 0:
                self.__dvtLib.WriteWithFPGAPattern(StartLba =StartLBA, blockCount = 0x1, SingleWrite =  False, pattern=4,usePreDefBlkCount = usePreDefBlkCount)
                self.__dvtLib.ReadWithFPGAPattern(StartLba =StartLBA, blockCount = 0x1, SingleRead =  False, pattern=4, usePreDefBlkCount = usePreDefBlkCount)
                self.__GetSequence.Run(ExpectSequence)

            if Random == 1:
                self.__dvtLib.WriteWithFPGAPattern(StartLba = StartLBA, blockCount = 0x1, SingleWrite =  True, pattern=4, usePreDefBlkCount= usePreDefBlkCount)
                self.__dvtLib.ReadWithFPGAPattern(StartLba = StartLBA, blockCount = 0x1, SingleRead =  True, pattern=4, usePreDefBlkCount = usePreDefBlkCount)
                self.__GetSequence.Run()
        ...
        if WriteType == 5:
            ...
            if Random == 0:
                self.__dvtLib.WriteWithFPGAPattern(StartLba =StartLBA, blockCount = 0x1, SingleWrite =  False, pattern=2, usePreDefBlkCount = usePreDefBlkCount)
                self.__dvtLib.ReadWithFPGAPattern(StartLba =StartLBA, blockCount = 0x1, SingleRead =  False, pattern=2, usePreDefBlkCount = usePreDefBlkCount)
                self.__GetSequence.Run(ExpectSequence)

            if Random == 1:
                self.__dvtLib.WriteWithFPGAPattern(StartLba = StartLBA, blockCount = 0x1, SingleWrite =  True, pattern=2, usePreDefBlkCount = usePreDefBlkCount)
                self.__dvtLib.ReadWithFPGAPattern(StartLba = StartLBA, blockCount = 0x1, SingleRead =  True, pattern=2, usePreDefBlkCount = usePreDefBlkCount)
                self.__GetSequence.Run(ExpectSequence)
        ...
        if WriteType == 6:
            ...
            self.__dvtLib.WriteWithFPGAPattern(StartLba =StartLBA, blockCount = BlockCount, SingleWrite =  False, pattern=DataType, usePreDefBlkCount = usePreDefBlkCount)
            self.__dvtLib.ReadWithFPGAPattern(StartLba =StartLBA, blockCount = BlockCount, SingleRead =  False, pattern=DataType, usePreDefBlkCount = usePreDefBlkCount)
        ...
        if WriteType == 15:
            ...
            if CiSequence == 0:
                if cmd20variables['CiStartBlock']% AU == 0:
                    rand = random.randrange(0,((cmd20variables['NumOfAU']-1) * AU))
                    cmd20variables['CiStartBlock'] = rand + cmd20variables['Lba_of_first_AU']
