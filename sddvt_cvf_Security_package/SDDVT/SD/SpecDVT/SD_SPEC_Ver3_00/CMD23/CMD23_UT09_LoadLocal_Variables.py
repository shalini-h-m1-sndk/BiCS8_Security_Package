...
# CVF SCRIPT                     : CMD23_UT09_LoadLocal_Variables.py
# DESCRIPTION                    :
# PRERQUISTE                     : [CMD23_UT12_SetFreq_Volt_Timeout]
...
import CMD23_UT12_SetFreq_Volt_Timeout as SetFreqVoltTimeout

# Global Variables

# Testcase Utility Class - Begins

class CMD23_UT09_LoadLocal_Variables(customize_log):
    def __init__(self, VTFContainer):
        ...
        self.__SetFreqVoltTo = SetFreqVoltTimeout.CMD23_UT12_SetFreq_Volt_Timeout(self.vtfContainer)

    # Testcase Utility Logic - Starts

    def Run(self, globalProjectValues, ret = 0):
        """
        Name : Run
        """
        ...
        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "Call setFreqVoltTimeOut \n")
        self.__SetFreqVoltTo.Run()

        # Read values into LocalVariables dictionary

        LocalVariables = {}
        LocalVariables['SendOCR']        = int(globalProjectValues['globalOCRArgValue'])
        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "SendOCR: %d" % LocalVariables['SendOCR'])

        LocalVariables['ExcpectOCR']     = int(globalProjectValues['globalOCRResValue'])
        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "ExcpectOCR: %d" % LocalVariables['ExcpectOCR'])

        LocalVariables['SendFirstOCR']   = int(globalProjectValues['globalOCRArgValue'])
        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "SendFirstOCR: %d" % LocalVariables['SendFirstOCR'])

        LocalVariables['SendNextOCR']    = int(globalProjectValues['globalOCRArgValue'])
