...
#
# SCRIPTER PARENT SCRIPT         : CallAllCMD23
# SCRIPTER SCRIPT                : DVT_SD3.00_3_3_1_4_CMD23_InvalidWrite.st3
# CVF CALL ALL SCRIPT            : CMD23_CA01_SDMode.xml
# CVF SCRIPT                     : CMD23_TC004_InvalidWrite.py
# DESCRIPTION                    : The purpose of this script is to test Invalid write end address for CMD23.
# PRERQUISTE                     : CMD23_UT08_LoadCMD23_Variables, CMD23_UT09_LoadLocal_Variables, CMD23_UT06_CMD23Write, CMD23_UT10_Reset
# STANDALONE EXECUTION           : Yes
# TEST ARGUMENTS                 : --test=CMD23_TC004_InvalidWrite --isModel=false --enable_console_log=1 --adapter=0
# AUTHOR [Referred Scripter/CTF] : Sivagurunathan
# REVIEWED BY                    : None
# DATE                           : 22-Nov-2022
# MODIFIED AUTHOR                : Sushmitha P.S
# UPDATED DATE                   : 28-Jun-2024
################################################################################
"""
...
class CMD23_TC004_InvalidWrite(TestCase.TestCase, customize_log):
    ...
    def setUp(self):
        ...
        2) Check and Switch to SD Protocol
    ...
    def Run(self, ret = 0):
        ...
        while LoopCounter < 100:
            ...
            if RandomBlockCount == 1:
                g_localVars["CMD23BlockCount"] = random.randrange(0,0xFFFF) + 1
            ...
            try:
                self.__dvtLib.WriteWithFPGAPattern(StartLba = g_localVars["CMD23StartBlock"], blockCount = g_localVars["CMD23BlockCount"] + 0xB, pattern=g_localVars["CMD23DataType"])
            ...
            try:
                self.__dvtLib.ReadWithFPGAPattern(g_localVars["CMD23StartBlock"], g_localVars["CMD23BlockCount"] + 0xB, pattern=g_localVars["CMD23DataType"])
            except ValidationError.CVFGenericExceptions as exc:
                self.__dvtLib.Expected_Failure_Check(exc, "CARD_OUT_OF_RANGE", "ReadWithFPGAPattern")
            else:
                raise ValidationError.TestFailError(self.fn, "Expected CARD_OUT_OF_RANGE error not occured")
        ...
        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "verifying valid write")
        ...
        Endlba = self.__sdCmdObj.calculate_endLba(startLba = g_localVars["CMD23StartBlock"], blockcount = g_localVars["blkCountCMD23"])
        self.__dvtLib.Erase( StartLba=g_localVars["CMD23StartBlock"], EndLba=Endlba, directCardAPI=True)

        # Do Multiple Read from CMD23StartBlock to CMD23BlockCount
        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "Performing Multiple Read operation")

        self.__dvtLib.ReadWithFPGAPattern(StartLba = g_localVars["CMD23StartBlock"],blockCount =  g_localVars["blkCountCMD23"], pattern=0, usePreDefBlkCount = True)
        self.__sdCmdObj.SetFrequency(Freq = 200000)

        self.__dvtLib.WriteWithFPGAPattern(StartLba = 0x0, blockCount = 0xFFFFFFFF, pattern=g_localVars["CMD23DataType"], usePreDefBlkCount = True)
    ...
    def test_CMD23_TC004_InvalidWrite(self):  # [Same in xml tag: <Test name="test_CMD23_TC004_InvalidWrite">]
