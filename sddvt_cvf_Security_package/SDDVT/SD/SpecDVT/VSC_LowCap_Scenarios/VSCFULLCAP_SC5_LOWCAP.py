...
# CTF SCRIPT                     : VSCFULLCAPSC5_Validation.py
# CVF CALL ALL SCRIPT            : None
# CVF SCRIPT                     : VSCFULLCAP_SC5_LOWCAP.py
...
# TEST ARGUMENTS                 : --test=VSCFULLCAP_SC5_LOWCAP --isModel=false --enable_console_log=1 --adapter=0
...
class VSCFULLCAP_SC5_LOWCAP(customize_log):
    ...
    def vsc10(self):
        ...
        self.__speedclass = 10
        frequency_vsc10 = [40, 80, 40]
        speedMode = ['SDR25', 'SDR50', 'DDR50']
        for index in range(self.__speedmodes):
            self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "VSC %d speedMode : %s" % (self.__speedclass, speedMode[index]))
            self.Compute_FATaddress()
            self.__globalProjectConfVar['globalSpeedMode'] = speedMode[index]
    ...
    def FATWrite(self):
        ...
        if ( self.__RUCOUNT == 16) : # increment FAT, DIR and CI address per 16 access
            self.__FAT1 = random.randrange(int(self.__FAT1start),int(self.__FAT1end),1) #self.__FAT1 = random.randrange(0x2000,0x3FDF,32)
            if self.__FAT1 + 1 > self.__FAT1end:
                self.__FAT1 = self.__FAT1end - 1
            self.__FAT2 = random.randrange(int(self.__FAT2start),int(self.__FAT2end),1) #self.__FAT2 = random.randrange(0x4000,0x5FDF,32)
            if self.__FAT2 + 1 > self.__FAT2end:
                self.__FAT2 = self.__FAT2end - 1
            self.__RUCOUNT = 0
...
class MainClass(TestCase.TestCase, customize_log):
    ...
    def test_VSCFULLCAP_SC5_LOWCAP(self):
        obj = VSCFULLCAP_SC5_LOWCAP(self.vtfContainer)
        obj.Run()
