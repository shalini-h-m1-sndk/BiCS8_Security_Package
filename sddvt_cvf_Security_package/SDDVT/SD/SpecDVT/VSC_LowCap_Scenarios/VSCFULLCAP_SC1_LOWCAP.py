...
# CTF SCRIPT                     : VSCFULLCAPSC1_Validation.py
# CVF CALL ALL SCRIPT            : None
# CVF SCRIPT                     : VSCFULLCAP_SC1_LOWCAP.py
...
# TEST ARGUMENTS                 : --test=VSCFULLCAP_SC1_LOWCAP --isModel=false --enable_console_log=1 --adapter=0
...
class VSCFULLCAP_SC1_LOWCAP(customize_log):
    ...
    def FATWrite(self):
        ...
        if (len(self.__Tfw) == 8):
            ...
            self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "TfwAvg [msec]: %f" % TfwMax)
            ...
            if (TfwMax > 750):
                    self.errorLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "********* Tfw max > 750ms : %s ********" % self.__globalProjectConfVar['globalSpeedMode'])
                    self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "TfwAvg [msec]: %f" % TfwMax)
        ...
        if ( self.__RUCOUNT == 16) : # increment FAT, DIR and CI address per 16 access
            self.__FAT1 = self.__FAT1 + 1
            if (self.__FAT1 +1 > self.__FAT1end):
                    self.__FAT1 = self.__FAT1start
            self.__FAT2 = self.__FAT2 + 1
...
class MainClass(TestCase.TestCase, customize_log):
    ...
    def test_VSCFULLCAP_SC1_LOWCAP(self):
        obj = VSCFULLCAP_SC1_LOWCAP(self.vtfContainer)
        obj.Run()
