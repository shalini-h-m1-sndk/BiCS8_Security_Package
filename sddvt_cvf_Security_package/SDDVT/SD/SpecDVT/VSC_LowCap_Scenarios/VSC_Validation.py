...
# CTF SCRIPT                     : VSC_Validation.py
# CVF CALL ALL SCRIPT            : None
# CVF SCRIPT                     : VSC_Validation.py
...
# TEST ARGUMENTS                 : --test=VSC_Validation --isModel=false --enable_console_log=1 --adapter=0
...
class VSC_Validation(customize_log):
    ...
    def vsc6(self):
        ...
        for index in range(0, self.__speedmodes):
    ...
    def Compute_FAT32_CSZIZE(self):
        ...
        self.__FAT1end = self.__FAT1start+ (int(SF)) -1
        self.__FAT2start = self.__FAT1end + 1
        self.__FAT2end = self.__FAT2start + (int(SF)) -1
    ...
    def UpdateDIR(self):
        ...
        #Write DIR
        self.__DIRStartLba = random.randint(int(self.__FAT2end+10),0x1FFFFF)  # Any address from end of FAT2/Bitmap to first 1GB of card capacity
    ...
    def FATWrite(self):
        ...
        self.__FAT1 = random.randrange(int(self.__FAT1start),int(self.__FAT1end),32) #self.__FAT1 = random.randrange(0x2000,0x3FDF,32)
        if self.__FAT1 + 32 > self.__FAT1end    :
            self.__FAT1 = self.__FAT1end-32
        self.__FAT2 = random.randrange(int(self.__FAT2start),int(self.__FAT2end),32)#self.__FAT2 = random.randrange(0x4000,0x5FDF,32)
        if self.__FAT2 + 32 > self.__FAT2end:
            self.__FAT2 = self.__FAT2end-32
        ...
        if (len(self.__Tfw) == 8):
            ...
            self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "%f " % TfwAvg)
            TfwMax = max(self.__Tfw)
            self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "%f" % TfwMax)
    ...
    def Precondition_card(self):
        ...
        while (RUaddress < (self.__cardMaxLba * (old_div(3, 4)))):
    ...
    def Measure_Pw(self):
        ...
        # AU selection
        #AU0 -> first AU after 2GB of card capacity
        #AU1 -> last AU of the card
        #AU2-AU11 -> randomly selected AU between 2GB and end capacity, other than AU0 and AU1
        AU_start = 0x600000
        selected_AU_list = []
        ...
        selected_AU_list.append(AU1)

        while len(selected_AU_list)<12:
            random_AU  = random.randint(AU0,AU1)
            if random_AU not in selected_AU_list:
                selected_AU_list.append(random_AU)

        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "selected AUs:%s" % (selected_AU_list))

        self.UpdateDIR()

        #"set free AU "
        RUnum = (old_div((self.__AUCapacity*1024*selected_AU_list[0]),512))
        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "Set Free AU : RUnum:0x%x" % RUnum)
        Argument = (0x78000000+RUnum)
        self.__sdCmdObj.Cmd20(Argument)
        time.sleep(0.250)        #250ms delay
        ...
        for loop_count in range(12): # 12 AUs to be checked for Pw
            Address_in_AU = selected_AU_list[loop_count]*AU_SIZE_Sectors
            SUList = []
            read_SUList = []
            ...
            self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "write_PerformanceZero_count: %s" % write_PerformanceZero_count)
            self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "read_PerformanceZero_count: %s" % Read_PerformanceZero_count)
            ...
            #"set free AU "
            if (loop_count < 11):
                RUnum = (old_div((self.__AUCapacity*1024*selected_AU_list[loop_count+1]),512))
                self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "Set Free AU : RUnum:0x%x" % RUnum)
                Argument = (0x78000000+RUnum)
    ...
    def Run(self):
        ...
        if (vsc in [30,60,90]):
            self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "VSC - Zero FAT Update.\n")
            self.__skipFATupdate = 1
            self.__speedmodes  = 2
            self.vsc30()
            self.__speedmodes  = 3
            self.vsc10()
            self.vsc6()
            #Perform VSC Zero FAT update
...
class MainClass(TestCase.TestCase, customize_log):
    ...
    def test_VSC_Validation(self):
        obj = VSC_Validation(self.vtfContainer)
        obj.Run()
