...
# SCRIPTER SCRIPT                : HCSD_V2_TC019_OCRvaluesInPowerCycleWithCMD0HV.st3
# CVF CALL ALL SCRIPT            : None
# CVF SCRIPT                     : HCSD_V2_TC019_OCRvaluesInPowerCycleWithCMD0HV.py
# DESCRIPTION                    :
# PRERQUISTE                     : HCSD_V2_UT01_AddressForWriteRead.py
# STANDALONE EXECUTION           : Yes
# TEST ARGUMENTS                 : --test=HCSD_V2_TC019_OCRvaluesInPowerCycleWithCMD0HV --isModel=false --enable_console_log=1 --adapter=0
# AUTHOR [Referred Scripter/CTF] : Sivagurunathan
# REVIEWED BY                    : None
# DATE                           : 30-May-2024
# UPDATED BY                     : Mahalaxmi Nalwad
# UPDATED DATE                   : 21-Aug-2024
################################################################################
"""
...
class HCSD_V2_TC019_OCRvaluesInPowerCycleWithCMD0HV(customize_log):
    ...
    def Run(self):
        ...
        #********* Power Cycle with CMD 0 Reset *********
        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "********* Power Cycle with CMD 0 Reset *********")
        ...
        for OCRcase1 in range(0, LoopCounter):
            self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "Loop OCRcase1 count is 4, Current iteration is %s" % (OCRcase1 + 1))
            ...
            # Reset card : No Responce - Reserved Bits 0-14 in ACMD 41 were modified!
            try:
                self.__dvtLib.Reset(mode=sdcmdWrap.CARD_MODE.Sd, ocr=ocr, cardSlot=0x1, sendCmd8=True,
                                    initInfo=None, rca=0x0, time=0x0, sendCMD0=0x1, bHighCapacity=False,
                                    bSendCMD58=False, version=0x1, VOLA=0x1, cmdPattern=0xAA,
                                    reserved=0x0)
            ...
            # Reset card
            try:
                self.__dvtLib.Reset(mode=sdcmdWrap.CARD_MODE.Sd, ocr=0xFF8000, cardSlot=0x1, sendCmd8=True,
                                    initInfo=None, rca=0x0, time=0x0, sendCMD0=0x1, bHighCapacity=False,
                                    bSendCMD58=False, version=0x1, VOLA=0x1, cmdPattern=0xAA,
                                    reserved=0x0)
            except ValidationError.CVFGenericExceptions as exc:
            ...
            self.__dvtLib.Reset(mode=sdcmdWrap.CARD_MODE.Sd, ocr=0x1FF8000, cardSlot=0x1, sendCmd8=True,
        ...
        for OCRcase2 in range(0, LoopCounter):
            self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "Loop OCRcase2 count is 4, Current iteration is %s" % (OCRcase2 + 1))
        ...
        for OCRcase3 in range(0, LoopCounter):
            self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "Loop OCRcase3 count is 4, Current iteration is %s" % (OCRcase3 + 1))
            ...
            try:
                ...
                raise ValidationError.TestFailError(self.fn, "Reset Failed with unexpected error")
        ...
        for OCRcase5 in range(0, LoopCounter):
            self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "Loop OCRcase5 count is 4, Current iteration is %s" % (OCRcase5 + 1))
            ...
            self.__sdCmdObj.Cmd8(argVal = 0x1AA)
        ...
        if (globalCardVoltage == "HV"):
            ...
            for OCRcase6 in range(0, LoopCounter):
                self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "Loop OCRcase6 count is 4, Current iteration is %s" % (OCRcase6 + 1))
            ...
            for OCRcase7 in range(0, LoopCounter):
                self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "Loop OCRcase7 count is 4, Current iteration is %s" % (OCRcase7 + 1))
                ...
                try:
                    self.__dvtLib.Reset(mode=sdcmdWrap.CARD_MODE.Sd, ocr=0xFF8000, cardSlot=0x1, sendCmd8=True,
                                        initInfo=None, rca=0x0, time=0x0, sendCMD0=0x1, bHighCapacity=True,
                                        bSendCMD58=False, version=0x0, VOLA=globalVOLAValue,
                                        cmdPattern=configPatternFiledInCMD8, reserved=0x0, expOCRVal=None)
                except ValidationError.CVFGenericExceptions as exc:
                    self.__dvtLib.Expected_Failure_Check(exc, "CARD_IS_NOT_RESPONDING", "Reset")
                else:
                    raise ValidationError.TestFailError(self.fn, "Expected CARD_IS_NOT_RESPONDING error not occurred")
                ...
                self.__dvtLib.Reset(mode=sdcmdWrap.CARD_MODE.Sd, ocr=0x1FF8000, cardSlot=0x1, sendCmd8=True,
                                    initInfo=None, rca=0x0, time=0x0, sendCMD0=0x1, bHighCapacity=True,
                                    bSendCMD58=False, version=0x0, VOLA=globalVOLAValue,
                                    cmdPattern=configPatternFiledInCMD8, reserved=0x0, expOCRVal=globalOCRResValue)
...
class MainClass(TestCase.TestCase, customize_log):
    ...
    def test_HCSD_V2_TC019_OCRvaluesInPowerCycleWithCMD0HV(self):
        obj = HCSD_V2_TC019_OCRvaluesInPowerCycleWithCMD0HV(self.vtfContainer)
        obj.Run()
