...
# CTF SCRIPT                     : OneGBWriteReadInSeqCQ.py
# CVF CALL ALL SCRIPT            : None
# CVF SCRIPT                     : OneGBWriteReadInSeqCQ.py
# DESCRIPTION                    : To execute one GB write and read in sequential CQ mode
# PRERQUISTE                     : None
# STANDALONE EXECUTION           : Yes
# TEST ARGUMENTS                 : --test=OneGBWriteReadInSeqCQ --isModel=false --enable_console_log=1 --adapter=0
...
class OneGBWriteReadInSeqCQ(TestCase.TestCase, customize_log):
    ...
    def WriteRead(self, Direction):
        ...
        StartTime = time.time()
        # queueing 32 tasks
        for TaskID in range(0, No_of_Tasks):

            StartBlockAddrList.append(StartBlockAddr)
            ...
            self.__dvtLib.CQTaskInformation(NumBlocks = NumBlocks, TaskID = TaskID, Priority = self.Priority, Direction = Direction)
        ...
        for TaskID in range(0, No_of_Tasks):
            ...
            if Direction == 0:
                ...
                self.__dvtLib.CQExecuteWriteTask(TaskID = TaskID, Pattern = Pattern, StartBlockAddr = StartBlockAddrList[TaskID], NumBlocks = NumBlocks)
            else:
                self.noiseLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, self.STD_LOG_SEND_CMD46_FOR_TASK_ID(TaskID))
                self.__dvtLib.CQExecuteReadTask(TaskID = TaskID, Pattern = Pattern, StartBlockAddr = StartBlockAddrList[TaskID],
                                                NumBlocks = NumBlocks, DoCompare = 0)

            # Check for the task completion status
            self.__dvtLib.GetTaskCompletionStatus(TaskID)

        EndTime = time.time()
        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "[OneGBWriteReadInSeqCQ]: One GB CQ %s is done in %s seconds" % ("read" if Direction else "write", EndTime - StartTime))


    def Run(self):
        """
        To execute one GB write and read in sequential CQ mode
        """

        # Enable the Sequential CQ Mode
        self.noiseLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, self.GL_ENABLE_SEQ_CQ_WITH_CACHE)
        self.__dvtLib.CQEnable(Set_CQ = True, Set_Cache = True, Set_Sequential_mode = True)

        # One GB write and read in sequential CQ mode
    ...
    def test_OneGBWriteReadInSeqCQ(self):  # [Same in xml tag: <Test name="test_OneGBWriteReadInSeqCQ">]
