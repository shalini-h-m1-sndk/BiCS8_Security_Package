...
# CTF SCRIPT                     : TC100_1_VolCQ_100Times_4K_Wr_4KRd_Cmp_Data_Diff_Pattern.py
# CVF CALL ALL SCRIPT            : None
# CVF SCRIPT                     : TC100_1_VolCQ_100Times_4K_Wr_4KRd_Cmp_Data_Diff_Pattern.py
# DESCRIPTION                    :
# PRERQUISTE                     : None
# STANDALONE EXECUTION           : Yes
# TEST ARGUMENTS                 : --test=TC100_1_VolCQ_100Times_4K_Wr_4KRd_Cmp_Data_Diff_Pattern --isModel=false --enable_console_log=1 --adapter=0
...
class TC100_1_VolCQ_100Times_4K_Wr_4KRd_Cmp_Data_Diff_Pattern(TestCase.TestCase, customize_log):
    ...
    def WriteOrRead(self, Direction):
        ...
        while True:
            ...
            if (Direction == 0) and (StartBlockAddrWrite < self.__cardMaxLba):
                ...
                self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "###[BREAK::1]: NumOfTimes * 4K done NumOfTaskQueued %d"%(NumOfTaskQueued))
                break
    ...
    def Run(self):
        ...
        for loop in range(0, ITERATION):
            ...
            while True:
                ...
                if StartBlockAddrWrite >= self.__cardMaxLba and (NumOfTaskQueued == NumOfTasksExecuted):
                    if TaskIDInExe != None :
                        #time.sleep(0.5)
                        #Check for the task completion
                        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "###[Run:5] TaskIDInExe = %d \n" %(TaskIDInExe))
                        self.CheckForTaskCompletion(TaskIDInExe)
                        TaskIDInExe = None

                    self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "###[Run:6] : ########## Finished writing 100 times")
                    ...
                    self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "###[Run:7] : Task Status Register value = 0x%X"%CQTaskStatusRegValue)
            ...
            self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "###[Run:7.1] : Sending CMD48..........")
            buf = ServiceWrap.Buffer.CreateBuffer(1, 0x00, isSector=True)
            self.__sdCmdObj.Cmd48(2,buf)
            ...
            while True:
                if StartBlockAddrRead < self.__cardMaxLba:
                    ...
                    self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "###[Run:8] : ###### Reading from StartBlockAddr = 0x%X ######"%StartBlockAddrRead)
                    self.WriteOrRead( Read_Direction)
                    self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "###[Run:9] : ###### Reading from StartBlockAddr = 0x%X - Done ######"%StartBlockAddrRead)

                if StartBlockAddrRead >= self.__cardMaxLba and (NumOfTaskQueued == NumOfTasksExecuted):
                    if TaskIDInExe != None :
                        #time.sleep(0.5)
                        #Check for the task completion
                        self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "###[Run:10] TaskIDInExe = %d \n" %(TaskIDInExe))
                        self.CheckForTaskCompletion(TaskIDInExe)
                        TaskIDInExe = None

                    self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "###[Run:11] : ########## Finished reading 100 times")
                    #Checking the Task Status register for value 0 after completing all the tasks
                    CQTaskStatusRegValue = self.__sdCmdObj.GetTaskStatusRegister()
                    self.infoLog(self.fn, currentframe().f_lineno, sys._getframe().f_code.co_name, "###[Run:12] : Task Status Register value = 0x%X"%CQTaskStatusRegValue)
                    if(CQTaskStatusRegValue !=0):
                        raise ValidationError.TestFailError(self.fn, "Run:CQTaskStatusRegister is not 0 after executing or aborting all the tasks")
    ...
    def test_TC100_1_VolCQ_100Times_4K_Wr_4KRd_Cmp_Data_Diff_Pattern(self):  # [Same in xml tag: <Test name="test_TC100_1_VolCQ_100Times_4K_Wr_4KRd_Cmp_Data_Diff_Pattern">]
